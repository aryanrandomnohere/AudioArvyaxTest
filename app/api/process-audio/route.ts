...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[v0] Processing file: ${audioFile.name}, size: ${buffer.length} bytes`)
    ...
    const leftDataUrl = `data:audio/wav;base64,${leftChannel.toString("base64")}`
    const rightDataUrl = `data:audio/wav;base64,${rightChannel.toString("base64")}`
...
async function separateAudioChannels(buffer: Buffer): Promise<{
  ...
  try {
    ...
    console.log(`[v0] Audio info: ${numChannels} channels, ${sampleRate}Hz, ${bitsPerSample} bits`)

    // Extract audio data
    const dataSize = buffer.readUInt32LE(dataOffset + 4)
    const audioData = buffer.subarray(dataOffset + 8, dataOffset + 8 + dataSize)
    ...
    if (numChannels === 1) {
      ...
      return handleStereoFile(audioData, sampleRate, bitsPerSample, bytesPerSample)
    } else {
      throw new Error(`Unsupported number of channels: ${numChannels}. Only mono and stereo files are supported.`)
    }
  } catch (error) {
...
function handleMonoFile(
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    for (let b = 0; b < bytesPerSample; b++) {
      ...
      rightOnlyData[stereoSampleOffset + bytesPerSample + b] = audioData[monoSampleOffset + b]
    }
  }

  const leftChannel = createStereoWavFile(leftOnlyData, sampleRate, bitsPerSample)
  const rightChannel = createStereoWavFile(rightOnlyData, sampleRate, bitsPerSample)
...
function handleStereoFile(
  ...
  for (let i = 0; i < samplesPerChannel; i++) {
    ...
    for (let b = 0; b < bytesPerSample; b++) {
      ...
      rightOnlyData[sampleOffset + bytesPerSample + b] = audioData[sampleOffset + bytesPerSample + b] // right channel
    }
  }
...
function findDataChunk(buffer: Buffer): number {
  ...
  if (riffHeader !== "RIFF") {
    throw new Error(`Invalid WAV file: missing RIFF header (found: ${riffHeader})`)
  }
...
function createStereoWavFile(audioData: Buffer, sampleRate: number, bitsPerSample: number): Buffer {
